           ...
           <el-scrollbar height="100%" style="width: 200px; padding: 20px; border-right: 1px solid #eee;">
                <div class="chat-anme" v-for="item in chatList" @click="GetChat(item.conver_id)">
                    <div>
                        ...
                        <el-image class="avatar" :src="item.users.sender.avatar"><template #error><span></span></template></el-image>
                        <div class="name">{{item.users.sender.nickname}}</div>
                    </div>
                    <el-icon style="width: 50px; height: 50px; margin-right: 10px;"><Sort /></el-icon>
                    <div>
                        <!-- <img :src="item.receiver.avatar"></img> -->
                        <el-image class="avatar" :src="item.users.receiver.avatar"><template #error><span></span></template></el-image>
                        <div class="name">{{item.users.receiver.nickname}}</div>
                    </div>
                </div>
           </el-scrollbar>
           <el-scrollbar height="100%" style="flex: 1; padding: 20px;">
                <div class="chat-left" v-for="item in chatContent">
                    <el-image class="avatar" :src="item.from_avatar"></el-image>
